...
public class EventProcessorAdvanced {
    ...
    public static void main(String[] args) {
        ...
        System.out.println("*** Waiting for events... (reports every 10 seconds)");

        final StreamsBuilder builder = new StreamsBuilder();
        KStream<String, String> source = builder.stream(group + "_sales");

        // Create 10-second tumbling windows
        TimeWindows tw = TimeWindows.ofSizeWithNoGrace(Duration.ofSeconds(10));

        // Aggregate total sum and count per product in each 10-second window
        source.groupByKey()
                .windowedBy(tw)
                .aggregate(
                        () -> new CountAndSum(),
                ...
                .toStream()
                .foreach(new MyProcessor()); // use external processor
